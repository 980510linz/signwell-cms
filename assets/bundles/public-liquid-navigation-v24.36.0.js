...
const PAGES=['index.html','topics.html','about.html','share.html','newsletter.html'];
...
function pageIndex(urlLike){const b=basename(urlLike);return b?PAGE_INDEX.get(b):0}
function retireLegacyShareQr(){
  ...
  app.querySelectorAll('#qrcode,.share-card .qr-box,#refreshQr,[data-share-qr],[data-qr-share]').forEach(el=>el.remove());
}
function setDirection(from,to){
  const a=pageIndex(from),b=pageIndex(to),forward=b>=a;
...
history.pushState=function(state,title,url){
  if(url!=null)setDirection(currentHref,new URL(url,location.href).href);
  const out=push(state,title,url);currentHref=location.href;queueMicrotask(retireLegacyShareQr);return out;
};
history.replaceState=function(state,title,url){
  if(url!=null)setDirection(currentHref,new URL(url,location.href).href);
  const out=replace(state,title,url);currentHref=location.href;queueMicrotask(retireLegacyShareQr);return out;
};
addEventListener('popstate',()=>{setDirection(currentHref,location.href);currentHref=location.href},{passive:true});
  ...
  if(app){
    ...
    new MutationObserver(()=>{
      ...
      raf=requestAnimationFrame(()=>{
        raf=0;root.classList.remove('sw-route-fallback-in');
        requestAnimationFrame(()=>{
          root.classList.add('sw-route-fallback-in');
          setTimeout(()=>root.classList.remove('sw-route-fallback-in'),280);
        });
      });
...
if(routeApp)new MutationObserver(()=>retireLegacyShareQr()).observe(routeApp,{childList:true,subtree:false});
retireLegacyShareQr();
...
function warmUrl(url){
  ...
  warmed.add(abs);
  fetch(abs,{method:'GET',cache:'force-cache',credentials:'same-origin'}).catch(()=>{});
}
function prewarm(){
  const c=navigator.connection||navigator.mozConnection||navigator.webkitConnection;
  if(c?.saveData||/slow-2g/i.test(String(c?.effectiveType||'')))return;
  PAGES.forEach((p,i)=>setTimeout(()=>warmUrl(p),i*90));
  ['public-data.json','articles/index.json','topics/index.json'].forEach((p,i)=>setTimeout(()=>warmUrl(p),480+i*90));
}
if('requestIdleCallback'in window)requestIdleCallback(prewarm,{timeout:900});
...
const pager=document.getElementById('pager')||document.querySelector('.pager');
if(pager){
  ...
  pager.addEventListener('pointerdown',()=>root.classList.add('sw-dock-interacting'),{passive:true});
...
window.SIGNWELL_PUBLIC_LIQUID_NAV=Object.freeze({version:VERSION,pages:PAGES.slice(),prewarm});
})();
