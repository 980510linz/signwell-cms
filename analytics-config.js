...
(()=>{'use strict';
  ...
  try{
    const q=String(new URLSearchParams(location.search).get('backend')||'').trim();
    const explicit=String(localStorage.getItem('signwell-backend-endpoint-explicit')||'').trim();
    if(valid(q)){endpoint=q;localStorage.setItem('signwell-backend-endpoint-explicit',q)}
    else if(valid(explicit))endpoint=explicit;
    localStorage.setItem('signwell-backend-endpoint',endpoint); // migrate stale legacy value to the active endpoint
  }catch(_){endpoint=FALLBACK_ENDPOINT}
  window.SIGNWELL_CANONICAL_BACKEND_ENDPOINT=FALLBACK_ENDPOINT;
  const cfg={enabled:valid(endpoint),endpoint,version:'24.36.3',bridgeProtocol:'23.9.92'};
